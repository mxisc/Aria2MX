...
- 刷新间隔、默认下载目录、浅色/深色模式、背景图片和面板密码设置
...
GitHub 上的 `mxisc/Aria2MX` 是 GitLab `mxisc/aria2mx` 的镜像仓库。镜像由 GitHub Actions 定时从 GitLab 拉取并强制同步，也可以在 GitHub Actions 页面手动触发 `Mirror from GitLab`。
